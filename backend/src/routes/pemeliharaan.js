...
router.post('/', async (req, res) => {
  try {
    ...
    const request_document_file_path = request_document_file_data ? await saveDataUrl(request_document_file_data, request_document_name, 'pemeliharaan') : null;
    const result = await pool.query(
      `INSERT INTO pemeliharaan (kode, judul, lokasi, titik_lokasi, kategori, deskripsi, tanggal, jenis_pekerjaan, urgensi, metode_pengadaan, request_document_name, request_document_file_data, request_document_file_path, created_by)
       VALUES ($1,$2,$3,$4,$5,$6, COALESCE($7, CURRENT_DATE), $8, COALESCE($9, 'sedang'), $10, $11, $12, $13, $14)
       RETURNING *`,
      [kode, judul, lokasi, titik_lokasi || null, kategori, deskripsi || null, tanggal || null, jenis_pekerjaan || null, urgensi || 'sedang', metode_pengadaan || null, request_document_name || null, request_document_file_data || null, request_document_file_path, req.user.id]
    );
...
router.put('/:id', requireRole(EDITOR_ROLES), async (req, res) => {
  try {
    ...
    const fields = ['judul','lokasi','titik_lokasi','kategori','deskripsi','tanggal','status',
                     ...
                     'jenis_pekerjaan','urgensi','metode_pengadaan','request_document_name','request_document_file_data','request_document_file_path','stage1_boq','stage1_boq_file_data','stage1_boq_file_path','stage1_hps','stage1_document_name','stage1_document_file_data','stage1_document_file_path',
                     'stage2_payment_method','stage2_vendor','stage2_invoice_number','stage2_invoice_date','stage2_invoice_amount','stage2_invoice_document_name','stage2_invoice_document_file_data','stage2_invoice_document_file_path',
                     'stage3_documentation_names','stage3_documentation_files','stage3_documentation_file_paths','stage3_bast_notes'];
    ...
    if (req.body.status === 'selesai') {
      ...
      // Frontend tidak boleh menghapusnya dengan null/kosong.
    ...
    if (req.body.stage3_documentation_files !== undefined) {
      const files = typeof req.body.stage3_documentation_files === 'string' ? JSON.parse(req.body.stage3_documentation_files) : req.body.stage3_documentation_files;
      const paths = [];
      for (const f of files || []) paths.push({ name: f.name, path: await saveDataUrl(f.data, f.name, 'pemeliharaan/stage3') });
      req.body.stage3_documentation_file_paths = JSON.stringify(paths);
    }

    const pathFields = {
      request_document_file_data: ['request_document_file_path','request_document_name'],
      stage1_boq_file_data: ['stage1_boq_file_path','stage1_boq'],
      stage1_document_file_data: ['stage1_document_file_path','stage1_document_name'],
      stage2_invoice_document_file_data: ['stage2_invoice_document_file_path','stage2_invoice_document_name'],
    };
    for (const [dataField, [pathField, nameField]] of Object.entries(pathFields)) {
      if (Object.prototype.hasOwnProperty.call(req.body, dataField) && req.body[dataField]) {
        req.body[pathField] = await saveDataUrl(req.body[dataField], req.body[nameField], 'pemeliharaan');
      }
    }

...
export default function RuangRapat() {
  ...
  const updateSelected=async(payload)=>{setSaving(true);try{const r=await api.put(`/ruang-rapat/${selected.id}`,payload);const updated=r.data.data;setItems(p=>p.map(x=>x.id===selected.id?updated:x));setSelected(updated);setError('');return updated;}catch(e){setError(e.response?.data?.message||'Gagal memperbarui booking.');throw e;}finally{setSaving(false);}};
  const cancelSelected=async()=>{if(!selected)return; if(!window.confirm(`Batalkan booking \"${selected.title}\"?\nBooking akan dihapus dari jadwal.`))return;setSaving(true);try{await api.delete(`/ruang-rapat/${selected.id}`);setItems(p=>p.filter(x=>x.id!==selected.id));setSelected(null);setError('');}catch(e){setError(e.response?.data?.message||'Gagal membatalkan booking.');}finally{setSaving(false);}};
  ...
  return <div className="menu-page menu-ruang-rapat max-w-[1400px] mx-auto">
    ...
    {selected&&<BookingModal booking={selected} saving={saving} onClose={()=>setSelected(null)} onUpdate={updateSelected} onCancel={cancelSelected} onView={(name,data)=>setViewer({open:true,name,data})}/>} 
  </div>;
}
...
function BookingModal({booking,saving,onClose,onUpdate,onCancel,onView}){
  const [edit,setEdit]=useState(false);
  const [form,setForm]=useState({title:booking.title||'',room:booking.room||ROOMS[0],pic:booking.pic||'',date:booking.date||'',start:booking.start||'',end:booking.end||''});
  const [name,setName]=useState(booking.surat_name||'');
  const [data,setData]=useState(booking.surat_file_data||'');
  const [status,setStatus]=useState(booking.status||'belum');
  const [msg,setMsg]=useState('');
  const pick=async e=>{const f=e.target.files?.[0];if(!f)return;if(!ALLOWED.includes(f.type)){setMsg('Surat harus PDF, JPG, atau PNG.');return;}if(f.size>MAX_FILE){setMsg('Ukuran surat maksimal 8 MB.');return;}setName(f.name);setData(await fileToDataUrl(f));setStatus('ditinjau');setMsg('Surat berhasil dipilih. Klik Simpan untuk mengubah warna/status.');};
  const saveEdit=async()=>{try{await onUpdate(form);setEdit(false);setMsg('Perubahan booking berhasil disimpan.');}catch(e){}};
  const saveSurat=async()=>{try{await onUpdate({surat_name:name,surat_file_data:data,status});setMsg('Surat/status berhasil disimpan.');}catch(e){}};
  const meta=STATUS_META[status]||STATUS_META.belum;
  return <ModalShell onClose={onClose} wide>
    <Header title={edit?'Edit Booking Ruang Rapat':'Detail Booking & Surat'} subtitle={edit?'Ubah agenda, PIC, ruangan, tanggal, atau jam booking.':'Kelola booking dan surat untuk mengubah indikator warna booking'} onClose={onClose} icon={edit?'edit_calendar':'description'}/>
    <div className="p-6 space-y-5">
      {edit ? <>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <Field label="Topik / Agenda"><input required value={form.title} onChange={e=>setForm({...form,title:e.target.value})} className="w-full h-11 px-3 rounded-lg border border-slate-300 bg-white text-sm outline-none focus:border-slate-900"/></Field>
          <Field label="Nama PIC"><input required value={form.pic} onChange={e=>setForm({...form,pic:e.target.value})} className="w-full h-11 px-3 rounded-lg border border-slate-300 bg-white text-sm outline-none focus:border-slate-900"/></Field>
          <Field label="Ruangan"><select value={form.room} onChange={e=>setForm({...form,room:e.target.value})} className="w-full h-11 px-3 rounded-lg border border-slate-300 bg-white text-sm outline-none focus:border-slate-900">{ROOMS.map(x=><option key={x}>{x}</option>)}</select></Field>
          <Field label="Tanggal"><input required type="date" value={form.date} onChange={e=>setForm({...form,date:e.target.value})} className="w-full h-11 px-3 rounded-lg border border-slate-300 bg-white text-sm outline-none focus:border-slate-900"/></Field>
          <Field label="Jam Mulai"><input required type="time" value={form.start} onChange={e=>setForm({...form,start:e.target.value})} className="w-full h-11 px-3 rounded-lg border border-slate-300 bg-white text-sm outline-none focus:border-slate-900"/></Field>
          <Field label="Jam Selesai"><input required type="time" value={form.end} onChange={e=>setForm({...form,end:e.target.value})} className="w-full h-11 px-3 rounded-lg border border-slate-300 bg-white text-sm outline-none focus:border-slate-900"/></Field>
        </div>
        <div className="flex justify-between gap-2 border-t pt-4"><button type="button" onClick={()=>setEdit(false)} className="px-4 py-2.5 rounded-lg border bg-white text-sm font-semibold">Kembali</button><button type="button" disabled={saving} onClick={saveEdit} className="px-5 py-2.5 rounded-lg bg-slate-900 text-white text-sm font-semibold">{saving?'Menyimpan...':'Simpan Perubahan'}</button></div>
      </> : <>
        <div className={`rounded-xl border p-4 ${meta.card}`}><div className="flex items-center justify-between gap-3"><div><div className="font-bold text-sm">{booking.title}</div><div className="text-xs mt-1">{booking.room} · {formatDateLabel(booking.date)} · {booking.start}–{booking.end}</div><div className="text-xs font-semibold mt-1">PIC: {booking.pic}</div></div><span className={`px-3 py-1.5 rounded-full border text-xs font-bold ${meta.badge}`}>{meta.label}</span></div></div>
        <div className="flex flex-wrap gap-2"><button type="button" disabled={saving} onClick={()=>setEdit(true)} className="px-4 py-2.5 rounded-lg bg-blue-600 text-white text-sm font-semibold">✎ Edit Booking</button><button type="button" disabled={saving} onClick={onCancel} className="px-4 py-2.5 rounded-lg bg-red-600 text-white text-sm font-semibold">✕ Cancel Booking</button></div>
        <div><label className="block text-xs font-bold text-slate-700 mb-2">Masukkan Surat</label><label className="relative flex items-center gap-4 p-5 rounded-xl border border-dashed border-slate-300 bg-white cursor-pointer hover:bg-slate-50"><span className="w-12 h-12 rounded-xl bg-slate-100 flex items-center justify-center"><span className="material-symbols-outlined text-slate-600">upload_file</span></span><div className="min-w-0 flex-1"><div className="text-sm font-semibold truncate">{name||'Pilih surat booking'}</div><div className="text-xs text-slate-500 mt-1">PDF, JPG, PNG · Maks. 8 MB</div></div><input type="file" className="absolute inset-0 opacity-0" accept="application/pdf,image/jpeg,image/png,.pdf,.jpg,.jpeg,.png" onChange={pick}/></label>{msg&&<p className="text-xs text-slate-500 mt-2">{msg}</p>}</div>
        <Field label="Status Surat"><select value={status} onChange={e=>setStatus(e.target.value)} className="w-full h-11 px-3 rounded-lg border border-slate-300 bg-white text-sm outline-none focus:border-slate-900"><option value="belum">🔴 Belum Ada Surat</option><option value="ditinjau">🔵 Surat Ditinjau</option><option value="diterima">🟢 Surat Diterima</option></select></Field>
        {data&&<button type="button" onClick={()=>onView(name,data)} className="text-xs font-semibold text-blue-700 hover:underline">Lihat surat yang dipilih</button>}
        <div className="flex justify-end gap-2 border-t pt-4"><button onClick={onClose} className="px-4 py-2.5 rounded-lg border bg-white text-sm font-semibold">Tutup</button><button disabled={saving} onClick={saveSurat} className="px-5 py-2.5 rounded-lg bg-slate-900 text-white text-sm font-semibold">{saving?'Menyimpan...':'Simpan Surat & Status'}</button></div>
      </>}
    </div>
  </ModalShell>
}

...
const emptyForm = { title:'', nomor_surat:'', room:ROOMS[0], pic:'', pic_phone:'', date:'', start:'', end:'', status:'belum' };
...
function validateBooking(f) {
  const errors = requireFields([['Topik / agenda', f.title], ['Nama PIC', f.pic], ['No. HP PIC', f.pic_phone], ['Tanggal', f.date], ['Jam mulai', f.start], ['Jam selesai', f.end]]);
...
export default function RuangRapat() {
  ...
  // Backend hanya mengizinkan kabag/PIC/admin membuat, mengedit, dan membatalkan booking.
  ...
  // Hanya ambil booking minggu yang sedang dilihat. weekRef mencegah respons minggu
  // lama (kalau navigasi diklik cepat) menimpa data minggu yang sedang tampil.
  const weekRef=useRef(weekStart); weekRef.current=weekStart;
  const loadBookings=async()=>{ const requested=weekStart; const end=new Date(`${weekStart}T00:00:00`); end.setDate(end.getDate()+6); try{const r=await api.get('/ruang-rapat',{params:{from:weekStart,to:localDateISO(end)}}); if(weekRef.current!==requested) return; setItems(r.data.data||[]);setError('');}catch(e){if(weekRef.current===requested) setError(e.response?.data?.message||'Gagal memuat jadwal dari database.');}finally{if(weekRef.current===requested) setLoading(false);} };
  useEffect(()=>{setLoading(true); loadBookings(); const timer=setInterval(loadBookings,60000); return()=>clearInterval(timer);},[weekStart]);
  useRuangRapatLive(loadBookings);
  // Daftar tidak membawa isi file surat; ambil detail lengkap saat booking dibuka.
  const openBooking=async(x)=>{setError('');try{const r=await api.get(`/ruang-rapat/${x.id}`);setSelected(r.data.data);setMode('detail');}catch(e){alert({title:'Gagal memuat detail',message:e.response?.data?.message||'Gagal memuat detail booking.',tone:'error'});}};
  ...
  const saveBooking=async(e)=>{e.preventDefault();if(saving)return;const errors=validateBooking(form);if(errors.length){await alert({title:'Data booking belum lengkap',intro:'Periksa data berikut:',message:errors,tone:'warning'});return;}setSaving(true);try{const r=await api.post('/ruang-rapat',form);setItems(p=>[...p,r.data.data].sort((a,b)=>`${a.date} ${a.start}`.localeCompare(`${b.date} ${b.start}`)));setShowBook(false);setForm({...emptyForm,date:''});setError('');toast(`Ruangan ${r.data.data.room} berhasil dibooking.`);}catch(e){alert({title:e.response?.status===409?'Jadwal bentrok':'Gagal menyimpan booking',message:e.response?.data?.message||'Gagal menyimpan booking.',tone:e.response?.status===409?'warning':'error'});}finally{setSaving(false);}};
  ...
  const cancelBooking=async()=>{if(!selected||saving)return;if(!(await confirm({title:'Batalkan booking?',message:`Booking "${selected.title}" (${selected.room}, ${formatDateLabel(selected.date)} ${selected.start}–${selected.end}) akan dibatalkan dan dihapus dari jadwal.`,confirmText:'Ya, Batalkan Booking',cancelText:'Tidak',tone:'danger',icon:'event_busy'})))return;setSaving(true);try{await api.delete(`/ruang-rapat/${selected.id}`);setItems(p=>p.filter(x=>x.id!==selected.id));setSelected(null);setError('');toast('Booking berhasil dibatalkan.');}catch(e){alert({title:'Gagal membatalkan booking',message:e.response?.data?.message||'Gagal membatalkan booking.',tone:'error'});}finally{setSaving(false);}};
  // Export Excel: Tanggal, Nomor Surat, Nama Rapat, PIC. Data diambil langsung dari
  // server sesuai rentang (bukan cuma minggu yang tampil). Default = minggu yang sedang dilihat.
  const [exportRange,setExportRange]=useState(null);
  const [exporting,setExporting]=useState(false);
  const range=exportRange||{mulai:weekDates[0],sampai:weekDates[6]};
  const exportExcel=async()=>{
    ...
    try{
      ...
      const rows=(r.data.data||[]).slice().sort((a,b)=>`${a.date} ${a.start}`.localeCompare(`${b.date} ${b.start}`));
      ...
      const headers=['Tanggal','Nomor Surat','Nama Rapat','PIC'];
      // mso-number-format:@ -> Excel memperlakukan sebagai teks (nomor surat & tanggal tidak diubah otomatis)
      const html=`<table><thead><tr>${headers.map(h=>`<th>${h}</th>`).join('')}</tr></thead><tbody>${rows.map(x=>`<tr>${[tgl(x.date),x.nomor_surat||'-',x.title,x.pic].map(v=>`<td style="mso-number-format:'\@'">${esc(v)}</td>`).join('')}</tr>`).join('')}</tbody></table>`;
      ...
      setError('');toast(`${rows.length} booking berhasil diexport ke Excel.`);
  ...
  return <div className="menu-page menu-ruang-rapat relative" style={{ fontFamily: 'Arial, Helvetica, sans-serif' }}>
    ...
    <div className="flex flex-col lg:flex-row justify-between items-start gap-4 mb-5"><div><h1 className="text-3xl font-bold">Jadwal & Booking Ruang Rapat</h1><p className="text-sm text-slate-600 mt-1 inline-flex items-center gap-1.5"><span className="w-1.5 h-1.5 rounded-full bg-emerald-500 animate-pulse"></span>Live real-time. Klik booking untuk Edit, surat, atau Cancel Booking.</p></div>{canManage&&<button onClick={()=>{setError('');setForm({...emptyForm,date:localDateISO()});setShowBook(true)}} className="bg-black text-white rounded-lg px-4 py-2.5 text-xs font-semibold shadow-sm">＋ Book Ruangan Rapat</button>}</div>
                      ...
                      <div className="space-y-2 min-h-[76px]">
                        {loading?<div className="text-[10px] text-slate-400 text-center py-6">Memuat...</div>:
                          ...
                          :cellItems.map(x=>{const m=STATUS_META[x.status]||STATUS_META.belum; return (
                            <button key={x.id} type="button" onClick={()=>openBooking(x)} className={`w-full text-left border rounded-lg p-2.5 shadow-sm transition hover:-translate-y-0.5 hover:shadow-md ${m.card}`}>
    ...
    {selected&&<BookingModal booking={selected} canManage={canManage} mode={mode==='detail'||canManage?mode:'detail'} setMode={(m)=>{setError('');setMode(m)}} saving={saving} error={error} onClose={()=>setSelected(null)} onUpdate={updateSelected} onCancel={cancelBooking} onView={(name,data)=>setViewer({open:true,name,data})}/>}
  </div>;
}

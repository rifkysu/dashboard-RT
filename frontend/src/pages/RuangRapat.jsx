...
export default function RuangRapat() {
  ...
  const openBooking=async(x)=>{setError('');try{const r=await api.get(`/ruang-rapat/${x.id}`);setSelected(r.data.data);setMode('detail');}catch(e){setError(e.response?.data?.message||'Gagal memuat detail booking.');}};
  ...
  const exportExcel=async()=>{
    ...
    if(!mulai||!sampai){setError('Isi tanggal mulai dan sampai untuk export.');return;}
    if(sampai<mulai){setError('Tanggal sampai harus sama atau setelah tanggal mulai.');return;}
    if((new Date(`${sampai}T00:00:00`)-new Date(`${mulai}T00:00:00`))/86400000>400){setError('Rentang export maksimal 400 hari.');return;}
    setExporting(true);
    try{
      ...
      if(!rows.length){setError('Tidak ada booking pada rentang tanggal tersebut.');return;}
      ...
      setError('');
    }catch(e){setError(e.response?.data?.message||'Gagal export jadwal ruang rapat.');}
